...
- **FR-017a**: ブラウザでの動作確認が必要なタスク（GitHub PR 画面、Slack 通知表示等）は、**人間が手動で**ブラウザ画面を確認し、スクリーンショットを PR の説明に含めなければならない（自動化は行わない。実装完了後の QA ステップとして実施する）
...
- **FR-020**: Max プラン利用時は Rate Controller がタスク間にクールダウン（デフォルト60秒）を挿入しなければならない。5時間ウィンドウあたりのタスク上限は環境変数 `MAX_TASKS_PER_WINDOW`（デフォルト150）で設定する。ウィンドウ残り枠が10%以下（デフォルト15タスク以下）になった時点で Slack に warn 通知を送信する
